...
void wypiszStruct(tperson *lista);
...
void main(){
   ...
   // dodajStruct(sciezka);
    ...
    while (opcja != 't' && opcja != 'T'){        
       // system("@cls||clear");
        printf("\n\t 1) Dodaj Listę do bd.txt\n");
	    printf("\t 2) Dodaj Structórę:\n");
	    printf("\t 3) Pokaż Plik %s\n", sciezka);
        printf("\t 4) Usuń Structórę\n");
	    printf("\t t/T) Koniec \n");
        printf("\n Podaj opcje: ");
	    scanf("%c", &opcja);	    
	    if (opcja == '1'){
	    ...
	    if (opcja == '2')  dodajStruct(sciezka);
	    if (opcja == '3')  pokazPlik(sciezka);
	    if (opcja == '4')  usunStruct(sciezka);
...
void dodajStruct(char *sciezka){
 tperson *nowy = malloc(sizeof(tperson));
        ...
        sprintf(nowy->email,"email%d@matrix.pl",rand()%66+25);
